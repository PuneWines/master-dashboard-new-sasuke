...
const SidebarContent = ({ onClose, isCollapsed = false, user, menuItems, attendanceOpen, onToggleAttendance }) => (
            ...
            <div key={item.label}>
              <button
                ...
                className={`flex items-center justify-between w-full py-3 px-4 rounded-xl transition-all duration-200 ${
                  attendanceOpen ? 'bg-indigo-800 text-white shadow-lg' : 'text-indigo-100 hover:bg-white/10 hover:text-white'
                }`}
              >
                ...
                {!isCollapsed && (attendanceOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />)}
              </button>
  ...
  return (
    <>
      ...
      <div className="hidden lg:block h-full flex-shrink-0">
        <SidebarContent user={user} menuItems={menuItems} attendanceOpen={attendanceOpen} onToggleAttendance={() => setAttendanceOpen(!attendanceOpen)} />
      </div>
      ...
      <div className={`fixed inset-0 z-[60] lg:hidden transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`fixed left-0 top-0 h-full z-[70] transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
          <SidebarContent onClose={() => setIsOpen(false)} user={user} menuItems={menuItems} attendanceOpen={attendanceOpen} onToggleAttendance={() => setAttendanceOpen(!attendanceOpen)} />
        </div>
      </div>
    </>
  );
