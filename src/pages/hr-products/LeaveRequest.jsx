...
const LeaveRequest = () => {
  ...
  const employeeId = localStorage.getItem("employeeId") || user?.joiningNo || user?.EmployeeID || user?.employeeId || '';
  ...
  const [formData, setFormData] = useState({
    ...
    employeeName: user?.Name || user?.name || user?.candidateName || '',
...
useEffect(() => {
  ...
  const fetchLeaveData = async () => {
    ...
    try {
      const response = await fetch(
        `${SCRIPT_URLS.HR_JOINING}?sheet=Leave Management&action=fetch`
      );
      ...
      const processedData = dataRows
        ...
        .filter(item => {
          ...
          const itemEmpId = (item.employeeId || '').toString().trim().toLowerCase();
          const itemEmpName = (item.employeeName || '').toString().trim().toLowerCase();
          
          // Match by Employee ID first, then by Name
          if (employeeId && itemEmpId === employeeId.toString().trim().toLowerCase()) return true;
          if (userName && itemEmpName === userName.trim().toLowerCase()) return true;
  ...
  const calculateLeaveBalance = () => {
    ...
    const approvedLeaves = leavesData.filter(leave => {
      ...
      const isSameEmployee = 
        (leave.employeeName?.toString().trim().toLowerCase() === userName.trim().toLowerCase()) ||
        (leave.employeeId?.toString().trim().toLowerCase() === employeeId?.toString().trim().toLowerCase());
      ...
      return isApproved && isSameEmployee && isSameMonth;
    });
