...
const SHEET_API_URL = `${SCRIPT_URLS.HR_JOINING}?sheet=USER&action=fetch`;
...
setSubmitting(true)
  ...
  // Persist user to Zustand authStore so it is available system-wide
  login(matchedUser);

  // Also explicitly store employeeId for consistent lookups across pages
  const employeeId = matchedUser.joiningNo || matchedUser.EmployeeID || matchedUser.Username || '';
  if (employeeId) {
