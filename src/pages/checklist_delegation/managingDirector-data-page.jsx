...
function ManagingDirectorDataPage() {
  ...
  // Toggle history view and reset filters
  const toggleHistory = () => {
  ...
  // Handle delete actual date for specific row in history table (batch-friendly, updateTaskData)
  const handleDeleteActual = async (itemId, rowIndex) => {
    try {
      const confirmDelete = window.confirm(
        "Are you sure you want to clear the Actual date (Column K) for this task?"
      );
      if (!confirmDelete) return;
      ...
      await fetchSheetData();
      setSuccessMessage("Actual date (Column K) cleared successfully!");
  ...
  const handleHistoryItemSelection = useCallback((id, isChecked) => {
  ...
  // Handle delete for selected history items (batch via updateTaskData)
  const handleHistoryDelete = async () => {
    ...
    try {
      ...
      console.error("Error clearing actual dates:", error);
          ...
          <div className="flex flex-wrap space-y-2 space-x-4">
            ...
            {showHistory && (
              <DeleteButton
                ...
                disabled={selectedHistoryItems.size === 0 || isDeletingHistory}
                      ...
                      <th className="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">
                        <input
                          ...
                          checked={
                            ...
                            filteredHistoryData.every((item) =>
                        ...
                        <tr key={history._id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <input
                              type="checkbox"
                              className="w-4 h-4 text-purple-600 rounded border-gray-300 focus:ring-purple-500"
                              checked={selectedHistoryItems.has(history._id)}
                              onChange={(e) =>
