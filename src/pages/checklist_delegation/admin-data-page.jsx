...
const CONFIG = {
  ...
  // Page configuration
  PAGE_CONFIG: {
    title: "Store Task Data",
    historyTitle: "Store Task History",
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (errMsg.includes("unknown action")) {
          ...
          if (!resJson.success) {
            throw new Error(
              resJson.error || "Failed to clear Actual date using fallback method"
            );
          }
        } else {
  ...
  const fetchSheetData = async () => {
    try {
      ...
      const response = await fetch(
        `https://docs.google.com/spreadsheets/d/1pso64b1nmDBPtq9V5Ay0L93smot03LKat_K0wka0XDY/gviz/tq?tqx=out:json&sheet=STORE`
      );
                    ...
                    {filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((history) => (
                        <tr key={history._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <button
                              onClick={() => handleDeleteActual(history._id, history._rowIndex)}
