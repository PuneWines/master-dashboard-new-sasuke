...
// Optimization: Memoized History Row Component
const HistoryRow = memo(({ history, isSelected, onSelect }) => {
  return (
    <tr
    ...
    >
      <td className="px-6 py-4 whitespace-nowrap">
        <input
          ...
          checked={isSelected}
          onChange={(e) => onSelect(e, history._id)}
        />
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
...
// Optimization: Memoized Task Row Component
const TaskRow = memo(({
  account,
  isSelected,
  isDeleteSelected,
    ...
    >
      <td className="px-6 py-4 whitespace-nowrap">
        <input
          ...
          className="w-4 h-4 text-purple-600 rounded border-gray-300 focus:ring-purple-500"
          checked={isSelected}
          onChange={(e) => onSelect(e, account._id)}
        />
      </td>
      {userRole === "admin" && (
        <td className="px-6 py-4 whitespace-nowrap bg-red-50">
          <input
            ...
            onChange={(e) => onDeleteSelect(e, account._id)}
          />
        </td>
      )}
      ...
      <td className="px-6 py-4 whitespace-nowrap bg-orange-50">
        <input
          ...
          disabled={!isSelected || !additionalDataValue}
          value={remarksDataValue || ""}
          onChange={(e) => onRemarksDataChange(account._id, e.target.value)}
          className="px-2 py-1 w-full rounded-md border border-gray-300 disabled:bg-gray-100 disabled:cursor-not-allowed"
        />
      </td>
      <td className="px-6 py-4 whitespace-nowrap bg-green-50">
        {account.image ? (
          ...
          <div className="flex flex-col space-y-2">
            <button
              type="button"
              onClick={() => onStartCamera(account._id)}
              disabled={!isSelected}
            ...
            <button
              ...
              onClick={() => {
                ...
                input.type = "file";
                input.accept = "image/*";
                input.onchange = (e) => onImageUpload(account._id, e);
                input.click();
              }}
              disabled={!isSelected}
              className={`flex items-center justify-center p-1 rounded-md ${account["col9"]?.toUpperCase() === "YES"
...
function SalesDataPage() {
  ...
  // Optimization: Memoized handlers for data changes
  ...
  const handleRemarksDataChange = useCallback((id, value) => {
    setRemarksData((prev) => ({
      ...prev,
      [id]: value,
    }));
  ...
  const handleDeleteCheckboxChange = useCallback((e, id) => {
  ...
  const startCamera = useCallback(async (accountId) => {
    try {
  ...
  const capturePhoto = useCallback(() => {
    if (videoRef.current && canvasRef.current && currentCameraAccountId) {
  ...
  }, [currentCameraAccountId, handleImageUpload, stopCamera]);
  ...
  const fetchSheetData = useCallback(async () => {
    try {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        //     // CHANGED: Include past dates AND today, but exclude future dates (tomorrow)
        //     const isPastOrToday = normalizedRowDate <= normalizedToday;
        ...
        // Add to history if Actual (Column K) is filled
        if (!isColumnKEmpty) {
  ...
  const handleHistoryDelete = async () => {
    ...
    try {
      // First attempt: use a dedicated clear action per row (if supported by Apps Script)
      ...
      for (const id of selectedHistoryItemsArray) {
        ...
        fd.append("action", "clearActualColumn");
        fd.append("rowIndex", String(item._rowIndex));
        fd.append("taskId", item["col1"] || "");
        ...
        try {
          ...
          console.error("clearActualColumn network error:", err);
          usedFallback = true;
          break;
        }
        ...
        try {
          ...
          console.warn("clearActualColumn non-JSON response, falling back.");
          usedFallback = true;
          break;
        }

        if (!json.success) {
          const msg = (json.error || text || "").toLowerCase();
          if (msg.includes("unknown action") || msg.includes("unsupported")) {
            console.warn(
              "clearActualColumn unsupported on server. Will use fallback."
            );
            usedFallback = true;
            break;
          }
          ...
          throw new Error(
            json.error || "Failed to clear Actual date via clearActualColumn"
          );
        }
      }

      // Fallback path: try multiple strategies to clear Column K
      if (usedFallback) {
          ...
          try {
            ...
            if (response1.ok) {
              ...
              try {
                ...
                if (responseText1.toLowerCase().includes("success")) {
              ...
              if (result1.success) {
                // Check if Column K was actually cleared
                const updateDetails = result1.updateDetails || [];
                const hasColumnKUpdate = updateDetails.some((detail) =>
                  (detail.updates || []).some(
                    (update) =>
                      update.toLowerCase().includes("actual") ||
                      update.toLowerCase().includes("column k") ||
                      update.toLowerCase().includes("date")
                  )
                );
        ...
        // Strategy 2: Use sentinel value if Strategy 1 didn't work
        if (!clearSuccess) {
            ...
            try {
              ...
              if (response2.ok) {
                ...
                try {
                  ...
                  if (responseText2.toLowerCase().includes("success")) {
            ...
            try {
              ...
              if (response3.ok) {
                ...
                try {
                  ...
                  if (responseText3.toLowerCase().includes("success")) {
        ...
        // Cleanup phase: If successful, ensure the cell is actually empty (not showing sentinel values)
        if (clearSuccess) {
          const cleanupData = selectedHistoryItemsArray
            .map((id) => {
              const item = historyData.find((h) => h._id === id);
              if (!item) return null;
              return {
                taskId: item["col1"],
                rowIndex: item._rowIndex,
                // Force truly empty value
                actualDate: "",
                clearSentinel: true, // Flag to tell Apps Script to clear any sentinel values
              };
            })
            .filter(Boolean);
          ...
          if (cleanupData.length > 0) {
            ...
            cleanupFormData.append("sheetName", CONFIG.SHEET_NAME);
            cleanupFormData.append("action", "updateTaskData");
            cleanupFormData.append("rowData", JSON.stringify(cleanupData));

            try {
              const cleanupResponse = await fetch(CONFIG.APPS_SCRIPT_URL, {
                method: "POST",
              ...
              if (cleanupResponse.ok) {
                ...
                let cleanupResult;
                try {
                  cleanupResult = JSON.parse(cleanupText);
                } catch (parseError) {
                  if (cleanupText.toLowerCase().includes("success")) {
                    cleanupResult = { success: true, message: cleanupText };
                  }
                }
              }
            } catch (cleanupError) {
      ...
      console.error("History delete (clear Actual) error:", error);
  ...
  const handlePendingDelete = async () => {
    ...
    try {
      ...
      formData.append("sourceSheetName", CONFIG.SHEET_NAME || "TLS");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Show success message immediately deleted - moving after successful post
      ...
      if (result.success) {
        // Update submitted items for history with real image URLs
        const submittedItemsForHistoryWithUrls = selectedItemsArray.map((id) => {
          const item = accountData.find((account) => account._id === id);
        ...
        setHistoryData((prev) => [...submittedItemsForHistoryWithUrls, ...prev]);
  ...
  return (
    <AdminLayout>
        ...
        <div className="flex flex-col gap-4 justify-between sm:flex-row sm:items-center">
          ...
          <div className="flex flex-wrap space-y-2 space-x-4">
            ...
            <button
              ...
              className="px-4 py-2 text-white bg-gradient-to-r from-blue-500 to-indigo-600 rounded-md hover:from-blue-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-75"
            >
              {isSwitchingView ? (
                <div className="flex items-center">
            ...
            {showHistory && (
              ...
              >
                <div className="flex items-center">
                  ...
                  <span>{isDeletingHistory ? "Clearing..." : "Delete"}</span>
                </div>
              </button>
            )}
                    ...
                    {filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((history) => (
                        <HistoryRow
                          key={history._id}
                          history={history}
                          isSelected={selectedHistoryItems.has(history._id)}
                          onSelect={handleHistoryCheckboxClick}
                        />
                      ))
                    ) : (
                      <tr>
                  ...
                  {filteredAccountData.length > 0 ? (
                    filteredAccountData.map((account) => (
                      <TaskRow
                        ...
                        isDeleteSelected={selectedDeleteItems.has(account._id)}
                        userRole={userRole}
                        additionalDataValue={additionalData[account._id]}
                        remarksDataValue={remarksData[account._id]}
                        onSelect={handleCheckboxClick}
                        onDeleteSelect={handleDeleteCheckboxChange}
                        onAdditionalDataChange={handleAdditionalDataChange}
                        onRemarksDataChange={handleRemarksDataChange}
      ...
      {/* Optimized Camera Modal Placement - Outside the list loop */}
      {cameraModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative z-[10000] bg-white rounded-lg p-4 max-w-md w-full mx-4 shadow-2xl">
            ...
            {/* Video Container */}
            <div className="relative mb-4 overflow-hidden bg-black rounded-md aspect-video">
              <video
                ref={videoRef}
                autoPlay
                playsInline
            ...
            <div className="flex justify-center w-full">
              <button
                ...
                className="flex relative items-center px-8 py-3 text-white bg-green-500 rounded-lg shadow-lg transition-all duration-200 transform z-[10001] hover:bg-green-600 hover:scale-105"
                style={{ zIndex: 10001 }}
              >
                <svg
